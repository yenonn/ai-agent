...
class AgentVerifiers:
    ...
    def architect_verifier(
        ...
        required_sections = [
            (
                "architecture overview",
                r"(?i)(architecture\s+overview|system\s+design|high-level\s+design)",
            ),
            (
                "component design",
                r"(?i)(component|module|service)\s+(design|breakdown|architecture)",
            ),
            (
                "api specifications",
                r"(?i)(api|endpoint|interface)\s+(spec|design|documentation)",
            ),
            ("security", r"(?i)(security|authentication|authorization)"),
            (
                "success criteria",
                r"(?i)(success\s+criteria|quality\s+gates|acceptance\s+criteria)",
            ),
        ]
        ...
        has_handoff_ready = (
            "handoff" in output.lower() or "ready for implementation" in output.lower()
        )
        ...
        if not has_handoff_ready:
            feedback_parts.append(
                "Include explicit handoff readiness statement with implementation priorities"
            )
    ...
    def coder_verifier(
        ...
        test_executed = any(
            "test" in str(call).lower() or "pytest" in str(call).lower()
        ...
        tests_passing = bool(
            re.search(
                r"(?i)(all\s+tests?\s+pass|tests?\s+successful|\d+\s+passed)", output
            )
        )

        # Check for implementation completeness
        has_error_handling = bool(
            re.search(r"(?i)(error\s+handling|try\s*[/{]|except|catch)", output)
        )
        has_documentation = bool(
            re.search(r"(?i)(docstring|documentation|commented|readme)", output)
        )

        # Check for "ready for review" marker
        ready_for_review = bool(
            re.search(r"(?i)(ready\s+for\s+review|implementation\s+complete)", output)
        )
    ...
    def reviewer_verifier(
        ...
        review_sections = [
            ...
            (
                "code quality",
                r"(?i)(code\s+quality|readability|maintainability|patterns)",
            ),
        ...
        has_approval = bool(
            re.search(r"(?i)(approved|lgtm|looks\s+good|ready\s+to\s+merge)", output)
        )
        has_changes_requested = bool(
            re.search(r"(?i)(changes?\s+required|must\s+fix|critical|blocking)", output)
        )
        ...
        if not has_decision:
            feedback_parts.append(
                "Provide explicit approval or change request decision"
            )
    ...
    def debug_verifier(
        ...
        checks = {
            "reproduced": bool(
                re.search(r"(?i)(reproduced|confirmed|verified\s+issue)", output)
            ),
            "root_cause": bool(
                re.search(
                    r"(?i)(root\s+cause|underlying\s+(issue|problem)|why\s+.+\s+occurs)",
                    output,
                )
            ),
            "code_path": bool(
                re.search(
                    r"(?i)(code\s+path|stack\s+trace|call\s+stack|line\s+\d+)", output
                )
            ),
            "fix_strategy": bool(
                re.search(r"(?i)(fix|solution|resolve|proposed\s+change)", output)
            ),
            "verification": bool(
                re.search(r"(?i)(verification|test\s+plan|how\s+to\s+verify)", output)
            ),
        }
        ...
        has_deep_analysis = bool(
            re.search(r"(?i)(because|due\s+to|caused\s+by|reason)", output)
        )
    ...
    def docs_verifier(
        ...
        checks = {
            "overview": bool(
                re.search(r"(?i)(overview|summary|introduction|about)", output)
            ),
            "examples": bool(
                re.search(r"(?i)(example|usage|how\s+to|getting\s+started)", output)
            ),
            "api_docs": bool(
                re.search(r"(?i)(api|endpoint|function|method|parameter)", output)
            ),
            "formatting": bool(
                re.search(r"(```|#\s+\w+|\*\*\w+\*\*)", output)
            ),  # Markdown indicators
            "complete_marker": bool(
                re.search(r"(?i)(documentation\s+complete|docs\s+updated)", output)
            ),
        }
    ...
    def devops_verifier(
        ...
        checks = {
            "pipeline": bool(
                re.search(r"(?i)(pipeline|ci/cd|workflow|actions)", output)
            ),
            "deployment": bool(
                re.search(r"(?i)(deploy|deployment|rollout|release)", output)
            ),
            "monitoring": bool(
                re.search(r"(?i)(monitoring|logging|metrics|alerts)", output)
            ),
            "config": bool(
                re.search(r"(?i)(environment|config|secrets|variables)", output)
            ),
            "tested": bool(
                re.search(r"(?i)(tested|verified|validated|successful)", output)
            ),
    ...
    def security_verifier(
        ...
        checks = {
            "vulnerability_scan": bool(
                re.search(
                    r"(?i)(vulnerabilit|security\s+scan|audit|assessment)", output
                )
            ),
            "severity_classification": has_severity,
            "auth_review": bool(
                re.search(
                    r"(?i)(authentication|authorization|access\s+control)", output
                )
            ),
            "input_validation": bool(
                re.search(r"(?i)(input\s+validation|sanitization|injection)", output)
            ),
            "remediation": bool(
                re.search(r"(?i)(remediation|fix|recommendation|mitigation)", output)
            ),
        }
